...
def processing_fun(img, folder_name):
   for i in range(1, 6):
      ...
      imageio.imwrite(output_folder + folder_name + '/' + filename, im_rgb)
      print("**************filename************", filename)

global thread_kill_flags

def processing(threadID, files, folder_name):
   for filename in files:
      start_milliseconds = str(int(round(time.time() * 1000)))
      path = input_folder + folder_name + '/' + filename
      Cimg = cv2.imread(path)
      print("=========111===========", path)
      if Cimg is not None:
         print("========222============")
         processing_fun(Cimg, folder_name);
      end_milliseconds = str(int(round(time.time() * 1000)))
      caculate_time_difference(start_milliseconds, end_milliseconds, filename)

class myThread (threading.Thread):
   def __init__(self, threadID, name, files, start_time, folder_name):
   ...
   def run(self):
      print("====================", self.name)
      processing(self.threadID, self.files, self.folder_name)
      self.stop()
...
def main(folder, start_time):
   ...
   for folder_name in os.listdir(folder):
      j += 1
      filenames = []
      for filename in os.listdir(input_folder + folder_name):
      ...
      for i in range(1, count_thread):
         ...
         # Create new threads
         k = j + i
         thread = myThread(k, "Thread_"+str(i), files, start_time, folder_name)
   ...
   if mode != 0:
      ...
      thread1 = myThread(count_thread, "Thread_"+str(count_thread), files, start_time, folder_name)
      thread1.start()
